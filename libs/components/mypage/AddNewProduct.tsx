...
const AddProduct = ({ initialValues, ...props }: any) => {
	...
	useEffect(() => {
		setInsertProductData({
			...
			dealAddress: getProductData?.getProduct ? getProductData?.getProduct?.dealAddress : '',
			productBarter: getProductData?.getProduct ? getProductData?.getProduct?.productBarter : false,
			productSharing: getProductData?.getProduct ? getProductData?.getProduct?.productSharing : false,
	...
	const doDisabledCheck = () => {
		if (
			...
			insertProductData.dealAddress === '' || // @ts-ignore
			insertProductData.productBarter === '' || // @ts-ignore
			insertProductData.productSharing === '' ||
					...
					<Stack className="config">
						<Stack className="description-box">
							...
							<Stack className="config-row">
								...
								<Stack className="price-year-after-price">
									...
									<input
										...
										className="description-input"
										placeholder={'Address'}
										value={insertProductData.dealAddress}
										onChange={({ target: { value } }) =>
											setInsertProductData({ ...insertProductData, dealAddress: value })
										}
									/>
								</Stack>
							</Stack>
							...
							<Stack className="config-row">
								...
								<Stack className="price-year-after-price">
									<Typography className="title">Sharing</Typography>
									<select
										className={'select-description'}
										value={insertProductData.productSharing ? 'yes' : 'no'}
										defaultValue={insertProductData.productSharing ? 'yes' : 'no'}
										onChange={({ target: { value } }) =>
											setInsertProductData({ ...insertProductData, productSharing: value === 'yes' })
										}
									>
										<option disabled={true} selected={true}>
											Select
